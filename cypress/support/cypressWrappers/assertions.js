...
class Assertions {
    ...
    assertElementsWithTextAreVisible(element, text) {
      element.each((el)=>{
        cy.wrap(el).contains(text).should('be.visible', {timeout: 90000, multiple: true});
      });
    }
    ...
    assertElementIsNotExist(element, timeout=10000) {
      element.should('not.exist', {timeout: timeout});
    }
    ...
    assertElementHaveValue(element, txt) {
      element.invoke('val').then((value) => {
        expect(value).to.contains(txt);
      });
    }
    ...
    assertTwoTextAreEquals(text1, text2) {
      cy.wrap(text1).should('eq', text2);
    }
    assertTwoTextAreNotEquals(text1, text2) {
      cy.wrap(text1).should('not.eq', text2);
    }
    ...
    assertElementIsNotEmpty(element, timeout = 90000) {
      element.should('not.be.empty', {timeout, multiple: true});
    }
  
    verifyElementTextMatchesRegex(element, regex) {
      expect(element.text().trim()).to.match(regex);
    }
  
    verifyElementTextDoesNotMatchesRegex(element, regex) {
      expect(element.text().trim()).not.to.match(regex);
    }
  
    verifyElementDoesNotHaveTag(element, tagName) {
      cy.get(element).should('not.have.prop', 'tagName', tagName);
    }
  }
